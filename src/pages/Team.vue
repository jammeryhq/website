<template>
  <Layout>
    <div>
      ...
      <ul
        class="mx-auto max-w-5xl my-10 grid grid-cols-1 gap-6 sm:grid-cols-2 md:grid-cols-3">
        <li
          v-for="edge in $page.plugins.edges"
          :key="edge.node.id"
          class="col-span-1 flex flex-col text-center bg-gray-100 rounded-lg shadow-none hover:shadow-2xl transition duration-500 ease-in-out">
          <div class="items-center justify-center flex flex-col p-8">
            <g-link
              :to="'team/' + edge.node.slug"
              class="p-2 block image-link rounded-full overflow-hidden">
              <g-image
                :src="edge.node.thumb"
...
.image-link {
  @apply rounded-full transition duration-500 ease-in-out ;
  &:hover {
